## 如何提取复用代码给多个 entry 使用

在 `Webpack` 中提取源码里被多个入口点复用的代码，例如一个 `utils` 文件，可以通过配置 **`optimization.splitChunks`** 来实现。`Webpack `会将这些频繁复用的模块提取出来，打包到一个独立的 `chunk` 中，使得浏览器可以`单独缓存这部分代码`，并在多个页面间共享使用，优化加载性能。
