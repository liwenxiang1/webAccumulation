...
1. CommonJS: 主要用于 `Node.js`，允许使用 `require()` 来加载模块和 `module.exports` 来导出模块。

1. AMD (Asynchronous Module Definition): 用于异步加载模块，并使用 `define` 方法来定义模块。

1. UMD (Universal Module Definition): 结合了 AMD 和 CommonJS 的特点，并支持全局变量定义的方式，使得模块可以在客户端和服务端上运行。
